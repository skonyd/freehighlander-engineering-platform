...
function summaryFromEvent(event) {
  ...
  return {
    ...
    criticalPathSpanIds: uniqueSortedPreserveOrder(payload.criticalPathSpanIds, 'criticalPathSpanIds'),
...
function compactDiagnostic(event) {
  ...
  return {
    ...
    status: firstString(payload.status, payload.outcome, event.execution?.status, event.execution?.result),
...
function compactGateOutcome(event) {
  ...
  return {
    ...
    timestamp: requireTimestamp(event.timestamp),
    nodeId: optionalText(event.node?.id),
    status: firstString(event.execution?.status, event.execution?.result, payload.status, payload.outcome),
    reasonCode: firstString(payload.reasonCode, payload.reason),
