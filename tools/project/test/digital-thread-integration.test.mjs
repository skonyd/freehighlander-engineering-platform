...
function fixture() {
  ...
  const releaseCandidate = {
    ...
    evidence: ['DEVELOPMENT', 'TESTING', 'SECURITY', 'BUILD', 'PROVENANCE'].map(
      (kind, index) => ({
  ...
  const entities = [
    ...
    { schemaVersion: 1, id: criterionId, version: 1, kind: 'ACCEPTANCE_CRITERION', label: 'Acceptance criterion', digest: digest('2') },
    { schemaVersion: 1, id: 'code-1', version: 1, kind: 'CODE_REVISION', label: 'Code revision', digest: digest('3'), revision: { repository, sha: headRevision } },
    { schemaVersion: 1, id: 'test-evidence-entity-1', version: 1, kind: 'TEST_EVIDENCE', label: 'Test evidence', digest: digest('4') },
    { schemaVersion: 1, id: 'security-evidence-entity-1', version: 1, kind: 'SECURITY_EVIDENCE', label: 'Security evidence', digest: digest('5') },
    { schemaVersion: 1, id: 'build-1', version: 1, kind: 'BUILD', label: 'Build', digest: digest('6'), revision: { repository, sha: buildRevision } },
    { schemaVersion: 1, id: 'release-entity-1', version: 1, kind: 'RELEASE', label: 'Release', digest: digest('7'), revision: { repository, sha: buildRevision } },
    { schemaVersion: 1, id: 'deployment-1', version: 1, kind: 'DEPLOYMENT', label: 'Deployment', digest: digest('8'), revision: { repository, sha: deploymentRevision } },
    { schemaVersion: 1, id: 'observation-1', version: 1, kind: 'RUNTIME_OBSERVATION', label: 'Runtime health', digest: digest('9') },
    { schemaVersion: 1, id: incident.id, version: 1, kind: 'INCIDENT', label: incident.title, digest: digest('a') },
  ];
  ...
  const lineage = {
    ...
    entities,
    relations: [
      { schemaVersion: 1, id: 'rel-1', kind: 'REFINES', relationClass: 'AUTHORITATIVE', from: ref('req-1'), to: ref(criterionId), evidence: evidence('rel-e1', 'b') },
      { schemaVersion: 1, id: 'rel-2', kind: 'IMPLEMENTED_BY', relationClass: 'AUTHORITATIVE', from: ref(criterionId), to: ref('code-1'), evidence: evidence('rel-e2', 'c') },
      { schemaVersion: 1, id: 'rel-3', kind: 'VERIFIED_BY', relationClass: 'AUTHORITATIVE', from: ref(criterionId), to: ref('test-evidence-entity-1'), evidence: evidence('rel-e3', 'd') },
      { schemaVersion: 1, id: 'rel-4', kind: 'SECURED_BY', relationClass: 'AUTHORITATIVE', from: ref('code-1'), to: ref('security-evidence-entity-1'), evidence: evidence('rel-e4', 'e') },
      { schemaVersion: 1, id: 'rel-5', kind: 'BUILT_FROM', relationClass: 'AUTHORITATIVE', from: ref('build-1'), to: ref('code-1'), evidence: evidence('rel-e5', 'f') },
      { schemaVersion: 1, id: 'rel-6', kind: 'RELEASES', relationClass: 'AUTHORITATIVE', from: ref('release-entity-1'), to: ref('build-1'), evidence: evidence('rel-e6', '1') },
      { schemaVersion: 1, id: 'rel-7', kind: 'DEPLOYED_AS', relationClass: 'AUTHORITATIVE', from: ref('deployment-1'), to: ref('release-entity-1'), evidence: evidence('rel-e7', '2') },
      { schemaVersion: 1, id: 'rel-8', kind: 'OBSERVED_BY', relationClass: 'AUTHORITATIVE', from: ref('deployment-1'), to: ref('observation-1'), evidence: evidence('rel-e8', '3') },
      { schemaVersion: 1, id: 'rel-9', kind: 'INVOLVED_IN', relationClass: 'AUTHORITATIVE', from: ref('observation-1'), to: ref(incident.id), evidence: evidence('rel-e9', '4') },
    ],
    authority: 'NONE',
