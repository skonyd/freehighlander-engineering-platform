...
test('architecture freeze rejects missing accepted ADRs, module contexts and unbounded loops', async () => {
  ...
  const missingLineage = structuredClone(await loadArchitectureContract(root));
  missingLineage.bounded_contexts.packages = missingLineage.bounded_contexts.packages.filter(
    (entry) => entry !== 'lineage',
  );
  assert.throws(() => assertArchitectureContract(missingLineage), /bounded contexts/);
