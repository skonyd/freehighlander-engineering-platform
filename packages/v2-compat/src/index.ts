...
export function testReviewRequired(input: TestReviewRoutingInput): boolean {
  ...
  if (input.reasons.some((reason) => reason === 'critical-or-deny-path' || reason === 'high-path')) {
    return true;
  }
...
export function validateProtocol(text: string, expectedSha?: string): ValidationResult {
  ...
  if (role === 'context-triage') {
    ...
    for (const field of ['ROOT_CAUSE_CANDIDATES', 'RELEVANT_LOG_LINES', 'LIKELY_FILES', 'FIX_CANDIDATES']) {
      if (!hasTopHeader(text, field)) errors.push(`missing ${field}`);
    }
...
export function validateTestReview(text: string, expectedSha?: string): ValidationResult {
  ...
  for (const field of [
    ...
    const matches = [
      ...coverage.matchAll(new RegExp(`^  ${field}: (.*)$`, 'gm')),
    ];
...
export async function stampCandidateAdjudication(
  ...
  if (
    containsReservedLine(controllerArtifact, [
      'PRODUCER',
      'CONFIG_HASH',
      'PRE_REVIEW_CONTENT_HASH',
    ])
  ) {
...
export async function stampContextTriageAdjudication(
  ...
  if (
    containsReservedLine(controllerArtifact, ['PRODUCER', 'CONFIG_HASH', 'TRIAGE_CONTENT_HASH'])
  ) {
...
export function planTestReviewAttempt(input: {
  ...
  const pendingRepair =
    input.state.lastOutcome === 'INSUFFICIENT' &&
    input.state.lastReviewedSha !== input.currentSha;
  ...
  return {
    ...
    pendingRepair,
    blockedByRoundLimit:
      pendingRepair && input.state.repairRounds >= input.maxRepairRounds,
  };
}
...
export function recordSuccessfulTestReview(input: {
  ...
  return {
    ...
    repairRounds:
      input.previousState.repairRounds + (input.pendingRepair ? 1 : 0),
  };
}
