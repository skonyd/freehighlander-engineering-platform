...
test('exact successful pure result is reusable and any execution identity drift is stale', () => {
  ...
  assert.deepEqual(evaluateNodeResultReuse(completed, { expectedIdentity: identity, sideEffecting: false }), {
    status: 'REUSABLE',
    reasons: [],
    authority: 'NONE',
  });
...
test('side-effecting success additionally requires exact idempotency evidence', () => {
  ...
  assert.deepEqual(withoutEvidence.reasons, ['side-effecting result requires idempotency evidence']);
...
test('mandatory join fails closed on missing stale semantic-negative and side-effect evidence gaps', () => {
  ...
  const joined = evaluateMandatoryJoin(
    [
      { nodeId: 'left-001', expectedIdentity: leftIdentity, sideEffecting: false },
      { nodeId: 'missing-001', expectedIdentity: buildNodeExecutionIdentity(identityInput({ nodeId: 'missing-001' })), sideEffecting: false },
      { nodeId: 'right-001', expectedIdentity: rightIdentity, sideEffecting: false },
