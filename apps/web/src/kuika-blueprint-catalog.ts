...
const DEFINITIONS: readonly CuratedBlueprintDefinition[] = [
  ...
  },
  {
    ...
    purpose: 'Investigate an incident, preserve evidence and produce bounded remediation candidates.',
    risk: 'HIGH',
  ...
  },
  {
    ...
    purpose: 'Prepare an architecture change with ADR, implementation, verification and independent review.',
    risk: 'HIGH',
...
export function getFhKuikaCuratedBlueprintV1(
  ...
  return (
    CURATED_BLUEPRINTS.find(
      (blueprint) => blueprint.id === id && blueprint.version === version,
    ) ?? null
  );
}
